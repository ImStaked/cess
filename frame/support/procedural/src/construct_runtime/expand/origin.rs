...
pub fn expand_outer_origin(
	...
	Ok(quote! {
		...
		pub struct Origin {
			caller: OriginCaller,
			filter: #scrate::cessp_std::rc::Rc<Box<dyn Fn(&<#runtime as #system_path::Config>::Call) -> bool>>,
		}

		#[cfg(not(feature = "std"))]
		impl #scrate::cessp_std::fmt::Debug for Origin {
			fn fmt(
				&self,
				fmt: &mut #scrate::cessp_std::fmt::Formatter,
			) -> #scrate::cessp_std::result::Result<(), #scrate::cessp_std::fmt::Error> {
		...
		impl #scrate::traits::OriginTrait for Origin {
			...
			fn add_filter(&mut self, filter: impl Fn(&Self::Call) -> bool + 'static) {
				...
				self.filter = #scrate::cessp_std::rc::Rc::new(Box::new(move |call| {
					f(call) && filter(call)
				}));
			}
			...
			fn reset_filter(&mut self) {
				...
				self.filter = #scrate::cessp_std::rc::Rc::new(Box::new(filter));
			}
		...
		impl #scrate::cessp_std::convert::TryFrom<OriginCaller> for #system_path::Origin<#runtime> {
			type Error = OriginCaller;
			fn try_from(x: OriginCaller)
				-> #scrate::cessp_std::result::Result<#system_path::Origin<#runtime>, OriginCaller>
			{
		...
		impl From<OriginCaller> for Origin {
			fn from(x: OriginCaller) -> Self {
				let mut o = Origin {
					caller: x,
					filter: #scrate::cessp_std::rc::Rc::new(Box::new(|_| true)),
				};
		...
		impl From<Origin> for #scrate::cessp_std::result::Result<#system_path::Origin<#runtime>, Origin> {
...
fn expand_origin_pallet_conversions(
	...
	quote! {
		...
		impl From<Origin> for #scrate::cessp_std::result::Result<#pallet_origin, Origin> {
			/// NOTE: converting to pallet origin loses the origin filter information.
			fn from(val: Origin) -> Self {
		...
		impl #scrate::cessp_std::convert::TryFrom<OriginCaller> for #pallet_origin {
			type Error = OriginCaller;
			fn try_from(
				x: OriginCaller,
			) -> #scrate::cessp_std::result::Result<#pallet_origin, OriginCaller> {
