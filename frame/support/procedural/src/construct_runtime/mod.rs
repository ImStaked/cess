...
fn construct_runtime_parsed(definition: RuntimeDefinition) -> Result<TokenStream2> {
	...
	let res = quote!(
		...
		#[derive(
			Clone, Copy, PartialEq, Eq, #scrate::cessp_runtime::RuntimeDebug,
			#scrate::scale_info::TypeInfo
		)]
		pub struct #name;
		impl #scrate::cessp_runtime::traits::GetNodeBlockType for #name {
			type NodeBlock = #node_block;
		}
		impl #scrate::cessp_runtime::traits::GetRuntimeBlockType for #name {
	...
	quote!(
		...
		impl #scrate::traits::PalletInfo for PalletInfo {
			fn index<P: 'static>() -> Option<usize> {
				let type_id = #scrate::cessp_std::any::TypeId::of::<P>();
				#(
					if type_id == #scrate::cessp_std::any::TypeId::of::<#names>() {
						return Some(#indices)
					}
				)*
