...
pub fn expand_error(def: &mut Def) -> proc_macro2::TokenStream {
	...
	let phantom_variant: syn::Variant = syn::parse_quote!(
		...
		#[codec(skip)]
		__Ignore(
			#frame_support::cessp_std::marker::PhantomData<(#type_use_gen)>,
			#frame_support::Never,
		)
	);
	...
	quote::quote_spanned!(error.attr_span =>
		impl<#type_impl_gen> #frame_support::cessp_std::fmt::Debug for #error_ident<#type_use_gen>
			#config_where_clause
		{
			fn fmt(&self, f: &mut #frame_support::cessp_std::fmt::Formatter<'_>)
				-> #frame_support::cessp_std::fmt::Result
			{
		...
		impl<#type_impl_gen> From<#error_ident<#type_use_gen>>
			for #frame_support::cessp_runtime::DispatchError
			#config_where_clause
		{
			fn from(err: #error_ident<#type_use_gen>) -> Self {
				...
				#frame_support::cessp_runtime::DispatchError::Module {
					index,
					error: err.as_u8(),
