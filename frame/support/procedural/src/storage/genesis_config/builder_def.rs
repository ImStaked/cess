			...
			if let Some(data) = data {
				blocks.push(match &line.storage_type {
					...
					StorageLineTypeDef::Map(map) => {
						...
						quote! {{
							#data
							let data: &#scrate::cessp_std::vec::Vec<(#key, #value_type)> = data;
					...
					StorageLineTypeDef::DoubleMap(map) => {
						...
						quote! {{
							#data
							let data: &#scrate::cessp_std::vec::Vec<(#key1, #key2, #value_type)> = data;
					...
					StorageLineTypeDef::NMap(map) => {
						...
						quote! {{
							#data
							let data: &#scrate::cessp_std::vec::Vec<(#key_tuple, #value_type)> = data;
							data.iter().for_each(|(k, v)| {
