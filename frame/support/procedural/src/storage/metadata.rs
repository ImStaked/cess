		...
		StorageLineTypeDef::Map(map) => {
			...
			quote! {
				#scrate::metadata::StorageEntryType::Map {
					hashers: #scrate::cessp_std::vec! [ #scrate::metadata::#hasher ],
...
fn default_byte_getter(
	...
	let struct_def = quote! {
		...
		>(pub #scrate::cessp_std::marker::PhantomData<(#runtime_generic #optional_comma_instance)>);

		#[cfg(feature = "std")]
		#[allow(non_upper_case_globals)]
		static #cache_name: #scrate::once_cell::sync::OnceCell<#scrate::cessp_std::vec::Vec<u8>> =
		...
		impl<#runtime_generic: #runtime_trait, #optional_instance_bound>
			...
			#where_clause
		{
			fn default_byte(&self) -> #scrate::cessp_std::vec::Vec<u8> {
	...
	let struct_instance = quote!(
		#struct_name::<#runtime_generic, #optional_instance>(#scrate::cessp_std::marker::PhantomData)
	);
...
pub fn impl_metadata(def: &DeclStorageDefExt) -> TokenStream {
	...
	for line in def.storage_lines.iter() {
		...
		let entry = quote! {
			#scrate::metadata::StorageEntryMetadata {
				...
				docs: #scrate::cessp_std::vec![ #( #docs ),* ],
			},
		};
	...
	let store_metadata = quote!(
		#scrate::metadata::PalletStorageMetadata {
			prefix: #prefix,
			entries: #scrate::cessp_std::vec![ #entries ],
		}
	);
