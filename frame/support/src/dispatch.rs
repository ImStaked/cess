...
pub use cessp_runtime::{traits::Dispatchable, DispatchError};
...
pub type DispatchResultWithPostInfo =
	cessp_runtime::DispatchResultWithInfo<crate::weights::PostDispatchInfo>;
...
pub type DispatchResult = Result<(), cessp_runtime::DispatchError>;

/// The error type contained in a `DispatchResultWithPostInfo`.
pub type DispatchErrorWithPostInfo =
	cessp_runtime::DispatchErrorWithPostInfo<crate::weights::PostDispatchInfo>;
...
/// implement the [`OnRuntimeUpgrade`](../cessp_runtime/traits/trait.OnRuntimeUpgrade.html) trait.
...
macro_rules! decl_module {
	...
	(@impl_on_initialize
			...
			for $module<$trait_instance$(, $instance)?> where $( $other_where_bounds )*
		{
			fn on_initialize(_block_number_not_used: <$trait_instance as $system::Config>::BlockNumber) -> $return {
				$crate::cessp_tracing::enter_span!($crate::cessp_tracing::trace_span!("on_initialize"));
				{ $( $impl )* }
			}
		}
	};
	...
	(@impl_on_runtime_upgrade
			...
			for $module<$trait_instance$(, $instance)?> where $( $other_where_bounds )*
		{
			fn on_runtime_upgrade() -> $return {
				$crate::cessp_tracing::enter_span!($crate::cessp_tracing::trace_span!("on_runtime_upgrade"));
				let pallet_name = <<
					$trait_instance
					as
					$system::Config
	...
	(@impl_on_finalize
			...
			for $module<$trait_instance$(, $instance)?> where $( $other_where_bounds )*
		{
			fn on_finalize(_block_number_not_used: <$trait_instance as $system::Config>::BlockNumber) {
				$crate::cessp_tracing::enter_span!($crate::cessp_tracing::trace_span!("on_finalize"));
				{ $( $impl )* }
			}
		}
	};
	...
	(@impl_on_idle
			...
			for $module<$trait_instance$(, $instance)?> where $( $other_where_bounds )*
		{
			fn on_idle($param1: $param1_ty, $param2: $param2_ty) -> $return {
				$crate::cessp_tracing::enter_span!($crate::cessp_tracing::trace_span!("on_idle"));
				{ $( $impl )* }
			}
		}
	};
	...
	(@impl_function
		...
		$vis fn $name(
			...
			$crate::cessp_tracing::enter_span!($crate::cessp_tracing::trace_span!(stringify!($name)));
			{ $( $impl )* }
			Ok(())
		}
	};
	...
	(@create_call_enum
			...
			where $( $other_where_bounds )*
		{
			#[doc(hidden)]
			#[codec(skip)]
			__PhantomItem($crate::cessp_std::marker::PhantomData<($trait_instance, $($instance)?)>, $crate::Never),
			$( $generated_variants )*
		}
	};
	...
	(@imp
		...
		pub struct $mod_type<
			$trait_instance: $trait_name
			$(<I>, $instance: $instantiable $( = $module_default_instance)?)?
		>($crate::cessp_std::marker::PhantomData<($trait_instance, $( $instance)?)>) where
			$( $other_where_bounds )*;
...
macro_rules! __impl_module_constants_metadata {
	...
	(GENERATE_CODE
			...
			$mod_type<$trait_instance $(, $instance)?> where $( $other_where_bounds )*
		{
			#[doc(hidden)]
			#[allow(dead_code)]
			pub fn pallet_constants_metadata() -> $crate::cessp_std::vec::Vec<$crate::metadata::PalletConstantMetadata> {
				...
				$crate::cessp_std::vec![
					$(
						$crate::metadata::PalletConstantMetadata {
							...
							docs: $crate::cessp_std::vec![ $( $doc_attr ),* ],
						}
					),*
				]
			}
...
mod tests {
	...
	impl PalletInfo for TraitImpl {
		fn index<P: 'static>() -> Option<usize> {
			...
			if type_id == cessp_std::any::TypeId::of::<Test>() {
				return Some(0)
			}
