...
#[doc(hidden)]
pub use cessp_runtime::traits::{BadOrigin, LookupError};

/// Declare an error type for a runtime module.
///
/// `decl_error!` supports only variants that do not hold any data. The dispatchable
/// functions return [`DispatchResult`](cessp_runtime::DispatchResult). The error type
...
macro_rules! decl_error {
	(
		...
		$( where $( $where_ty: $where_bound ),* )?
		{
			...
			#[codec(skip)]
			__Ignore(
				$crate::cessp_std::marker::PhantomData<($generic, $( $inst_generic)?)>,
				$crate::Never,
			),
			$(
				$( #[doc = $doc_attr] )*
		...
		impl<$generic: $trait $(, $inst_generic: $instance)?> $crate::cessp_std::fmt::Debug
			for $error<$generic $(, $inst_generic)?>
		$( where $( $where_ty: $where_bound ),* )?
		{
			fn fmt(&self, f: &mut $crate::cessp_std::fmt::Formatter<'_>) -> $crate::cessp_std::fmt::Result {
		...
		impl<$generic: $trait $(, $inst_generic: $instance)?> From<$error<$generic $(, $inst_generic)?>>
			for $crate::cessp_runtime::DispatchError
		$( where $( $where_ty: $where_bound ),* )?
		{
			fn from(err: $error<$generic $(, $inst_generic)?>) -> Self {
				...
				$crate::cessp_runtime::DispatchError::Module {
					index,
					error: err.as_u8(),
