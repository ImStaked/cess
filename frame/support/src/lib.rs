...
#[cfg(feature = "std")]
pub use serde;
pub use cessp_core::Void;
#[doc(hidden)]
pub use cessp_io::{self, storage::root as storage_root};
#[doc(hidden)]
pub use cessp_runtime::RuntimeDebug;
#[cfg(feature = "std")]
#[doc(hidden)]
...
pub mod unsigned {
	#[doc(hidden)]
	pub use crate::cessp_runtime::traits::ValidateUnsigned;
	#[doc(hidden)]
	pub use crate::cessp_runtime::transaction_validity::{
...
pub use cessp_runtime::{self, print, traits::Printable, ConsensusEngineId};

use codec::{Decode, Encode};
use scale_info::TypeInfo;
use cessp_runtime::TypeId;
...
macro_rules! parameter_types {
	...
	(IMPL_STORAGE $name:ident, $type:ty, $value:expr) => {
		impl $name {
			...
			#[allow(unused)]
			pub fn key() -> [u8; 16] {
				$crate::cessp_io::hashing::twox_128(
...
macro_rules! ord_parameter_types {
	...
	(IMPL $name:ident , $type:ty , $value:expr) => {
		impl $crate::traits::SortedMembers<$type> for $name {
			...
			fn sorted_members() -> $crate::cessp_std::prelude::Vec<$type> { vec![$value] }
			fn count() -> usize { 1 }
...
macro_rules! runtime_print {
	($($arg:tt)+) => {
		{
			use core::fmt::Write;
			let mut w = $crate::cessp_std::Writer::default();
			let _ = core::write!(&mut w, $($arg)+);
			$crate::cessp_io::misc::print_utf8(&w.inner())
		}
	}
}

/// Print out the debuggable type.
pub fn debug(data: &impl cessp_std::fmt::Debug) {
	runtime_print!("{:?}", data);
}
...
pub mod tests {
	...
	fn double_map_basic_insert_remove_remove_prefix_should_work() {
		new_test_ext().execute_with(|| {
			...
			assert!(matches!(
				...
				cessp_io::KillStorageResult::AllRemoved(0), // all in overlay
			));
			assert_eq!(DoubleMap::get(&key1, &key2), 0u64);
	...
	fn check_storage_parameter_type_works() {
		TestExternalities::default().execute_with(|| {
			assert_eq!(cessp_io::hashing::twox_128(b":StorageParameter:"), StorageParameter::key());
...
pub mod pallet_prelude {
	...
	pub use cessp_std::marker::PhantomData;
}
...
/// * `#[cfg(feature = "std")]`
///
/// Macro will implement `cessp_runtime::BuildModuleGenesisStorage` using `()` as second generic
...
/// `Pallet<T>`, and some optional where clause.
///
/// NOTE: There is also `cessp_runtime::traits::SignedExtension` that can be used to add some
...
///
/// 	#[derive(codec::Encode, cessp_runtime::RuntimeDebug)]
/// 	#[cfg_attr(feature = "std", derive(codec::Decode))]
/// 	pub enum InherentError {
/// 	}
///
/// 	impl cessp_inherents::IsFatalError for InherentError {
/// 		fn is_fatal_error(&self) -> bool {
/// 			unimplemented!();
/// 		}
/// 	}
///
/// 	pub const INHERENT_IDENTIFIER: cessp_inherents::InherentIdentifier = *b"testpall";
/// }
/// ```
///
