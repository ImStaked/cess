...
use cessp_runtime::generic::{Digest, DigestItem};
pub use cessp_runtime::TransactionOutcome;
...
pub fn with_transaction<R>(f: impl FnOnce() -> TransactionOutcome<R>) -> R {
	use cessp_io::storage::{commit_transaction, rollback_transaction, start_transaction};
...
pub trait StorageDoubleMap<K1: FullEncode, K2: FullEncode, V: FullCodec> {
	...
	fn remove_prefix<KArg1>(k1: KArg1, limit: Option<u32>) -> cessp_io::KillStorageResult
	where
...
pub trait StorageNMap<K: KeyGenerator, V: FullCodec> {
	...
	fn remove_prefix<KP>(partial_key: KP, limit: Option<u32>) -> cessp_io::KillStorageResult
	where
		K: HasKeyPrefix<KP>;
...
impl<T, OnRemoval: PrefixIteratorOnRemoval> Iterator for PrefixIterator<T, OnRemoval> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			let maybe_next = cessp_io::storage::next_key(&self.previous_key)
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			let maybe_next = if self.fetch_previous_key {
				...
				cessp_io::default_child_storage::next_key(
...
pub trait StoragePrefixedMap<Value: FullCodec> {
	...
	fn remove_all(limit: Option<u32>) -> cessp_io::KillStorageResult {
		cessp_io::storage::clear_prefix(&Self::final_prefix(), limit)
	}
	...
	fn translate_values<OldValue: Decode, F: FnMut(OldValue) -> Option<Value>>(mut f: F) {
		...
		while let Some(next) =
			cessp_io::storage::next_key(&previous_key).filter(|n| n.starts_with(&prefix))
		{
			previous_key = next;
...
pub trait StorageDecodeLength: private::Sealed + codec::DecodeLength {
	...
	fn decode_len(key: &[u8]) -> Option<usize> {
		...
		let mut data = [0u8; 5];
		let len = cessp_io::storage::read(key, &mut data, 0)?;
	...
	StorageValueT: generator::StorageValue<T>,
{
	fn try_append<LikeI: EncodeLike<I>>(item: LikeI) -> Result<(), ()> {
		...
		if current < bound {
			...
			cessp_io::storage::append(&key, item.encode());
			Ok(())
		} else {
			Err(())
		}
	...
	StorageDoubleMapT: generator::StorageDoubleMap<K1, K2, T>,
{
	...
	>(
		...
		if current < bound {
			...
			cessp_io::storage::append(&double_map_key, item.encode());
			Ok(())
		} else {
			Err(())
		}
...
pub fn storage_prefix(pallet_name: &[u8], storage_name: &[u8]) -> [u8; 32] {
	let pallet_hash = cessp_io::hashing::twox_128(pallet_name);
	let storage_hash = cessp_io::hashing::twox_128(storage_name);
...
mod test {
	...
	use cessp_core::hashing::twox_128;
