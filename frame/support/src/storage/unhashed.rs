...
pub fn get<T: Decode + Sized>(key: &[u8]) -> Option<T> {
	cessp_io::storage::get(key).and_then(|val| {
...
pub fn put<T: Encode + ?Sized>(key: &[u8], value: &T) {
	value.using_encoded(|slice| cessp_io::storage::set(key, slice));
}
...
pub fn exists(key: &[u8]) -> bool {
	cessp_io::storage::exists(key)
}

/// Ensure `key` has no explicit entry in storage.
pub fn kill(key: &[u8]) {
	cessp_io::storage::clear(key);
}

/// Ensure keys with the given `prefix` have no entries in storage.
pub fn kill_prefix(prefix: &[u8], limit: Option<u32>) -> cessp_io::KillStorageResult {
	cessp_io::storage::clear_prefix(prefix, limit)
}

/// Get a Vec of bytes from storage.
pub fn get_raw(key: &[u8]) -> Option<Vec<u8>> {
	cessp_io::storage::get(key)
}
...
pub fn put_raw(key: &[u8], value: &[u8]) {
	cessp_io::storage::set(key, value)
}
