...
use cessp_arithmetic::traits::Saturating;
use cessp_runtime::traits::AtLeast32BitUnsigned;
...
pub trait OnRuntimeUpgradeHelpersExt {
	...
	fn get_temp_storage<T: codec::Decode>(at: &str) -> Option<T> {
		cessp_io::storage::get(&Self::storage_key(at))
	...
	fn set_temp_storage<T: codec::Encode>(data: T, at: &str) {
		cessp_io::storage::set(&Self::storage_key(at), &data.encode());
	}
}
...
pub trait Hooks<BlockNumber> {
	...
	///
	/// Implement this and use any of the `Offchain` `cessp_io` set of APIs
...
#[cfg(feature = "std")]
pub trait GenesisBuild<T, I = ()>: Default + cessp_runtime::traits::MaybeSerializeDeserialize {
	/// The build function is called within an externalities allowing storage APIs.
	/// Thus one can write to storage using regular pallet storages.
	fn build(&self);

	/// Build the storage using `build` inside default storage.
	fn build_storage(&self) -> Result<cessp_runtime::Storage, String> {
	...
	fn assimilate_storage(&self, storage: &mut cessp_runtime::Storage) -> Result<(), String> {
...
mod tests {
	...
	fn on_idle_round_robin_works() {
		static mut ON_IDLE_INVOCATION_ORDER: cessp_std::vec::Vec<&str> = cessp_std::vec::Vec::new();
