...
pub use cessp_runtime::transaction_validity::TransactionPriority;
...
impl OneOrMany<DispatchClass> for DispatchClass {
	type Iter = cessp_std::iter::Once<DispatchClass>;
	fn into_iter(self) -> Self::Iter {
		cessp_std::iter::once(self)
	}
}

impl<'a> OneOrMany<DispatchClass> for &'a [DispatchClass] {
	type Iter = cessp_std::iter::Cloned<cessp_std::slice::Iter<'a, DispatchClass>>;
	fn into_iter(self) -> Self::Iter {
		self.iter().cloned()
	}
}
...
impl cessp_runtime::traits::Printable for PostDispatchInfo {
	fn print(&self) {
		"actual_weight=".print();
...
#[cfg(feature = "std")]
impl<Call: Encode, Extra: Encode> GetDispatchInfo for cessp_runtime::testing::TestXt<Call, Extra> {
	fn get_dispatch_info(&self) -> DispatchInfo {
...
pub struct IdentityFee<T>(cessp_std::marker::PhantomData<T>);
