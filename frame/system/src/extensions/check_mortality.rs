...
pub struct CheckMortality<T: Config + Send + Sync>(Era, cessp_std::marker::PhantomData<T>);

impl<T: Config + Send + Sync> CheckMortality<T> {
	/// utility constructor. Used only in client/factory code.
	pub fn from(era: Era) -> Self {
		Self(era, cessp_std::marker::PhantomData)
	}
}

impl<T: Config + Send + Sync> cessp_std::fmt::Debug for CheckMortality<T> {
	#[cfg(feature = "std")]
