...
use cessp_std::map;
use cessp_std::{fmt::Debug, marker::PhantomData, prelude::*};
...
use cessp_core::{storage::well_known_keys, ChangesTrieConfiguration};

#[cfg(feature = "std")]
use frame_support::traits::GenesisBuild;
#[cfg(any(feature = "std", test))]
use cessp_io::TestExternalities;
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// - 1 call to `can_set_code`: `O(S)` (calls `cessp_io::misc::runtime_version` which is
		///   expensive).
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig {
		fn build(&self) {
			...
			cessp_io::storage::set(well_known_keys::CODE, &self.code);
			cessp_io::storage::set(well_known_keys::EXTRINSIC_INDEX, &0u32.encode());
			if let Some(ref changes_trie_config) = self.changes_trie_config {
				cessp_io::storage::set(
					well_known_keys::CHANGES_TRIE_CONFIG,
					&changes_trie_config.encode(),
				);
			}
...
impl GenesisConfig {
	...
	pub fn build_storage<T: Config>(&self) -> Result<cessp_runtime::Storage, String> {
	...
	pub fn assimilate_storage<T: Config>(
		&self,
		storage: &mut cessp_runtime::Storage,
	) -> Result<(), String> {
...
/// happened.
#[derive(cessp_runtime::RuntimeDebug, Encode, Decode, TypeInfo)]
#[cfg_attr(feature = "std", derive(PartialEq))]
pub struct LastRuntimeUpgradeInfo {
	pub spec_version: codec::Compact<u32>,
	pub spec_name: cessp_runtime::RuntimeString,
}
...
pub struct EnsureRoot<AccountId>(cessp_std::marker::PhantomData<AccountId>);
...
pub struct EnsureSigned<AccountId>(cessp_std::marker::PhantomData<AccountId>);
...
pub struct EnsureSignedBy<Who, AccountId>(cessp_std::marker::PhantomData<(Who, AccountId)>);
impl<
...
pub struct EnsureNone<AccountId>(cessp_std::marker::PhantomData<AccountId>);
impl<O: Into<Result<RawOrigin<AccountId>, O>> + From<RawOrigin<AccountId>>, AccountId>
...
pub struct EnsureNever<T>(cessp_std::marker::PhantomData<T>);
...
pub struct EnsureOneOf<AccountId, L, R>(cessp_std::marker::PhantomData<(AccountId, L, R)>);
impl<
		AccountId,
		O: Into<Result<RawOrigin<AccountId>, O>> + From<RawOrigin<AccountId>>,
		L: EnsureOrigin<O>,
...
impl<T: Config> Pallet<T> {
	...
	pub fn finalize() -> T::Header {
		...
		let storage_root = T::Hash::decode(&mut &cessp_io::storage::root()[..])
			.expect("Node is configured to use the same hash; qed");
		let storage_changes_root = cessp_io::storage::changes_root(&parent_hash.encode());
	...
	pub fn externalities() -> TestExternalities {
		TestExternalities::new(cessp_core::storage::Storage {
			top: map![
	...
	pub fn can_set_code(code: &[u8]) -> Result<(), cessp_runtime::DispatchError> {
		let current_version = T::Version::get();
		let new_version = cessp_io::misc::runtime_version(&code)
