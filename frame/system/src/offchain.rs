...
pub struct SubmitTransaction<T: SendTransactionTypes<OverarchingCall>, OverarchingCall> {
	_phantom: cessp_std::marker::PhantomData<(T, OverarchingCall)>,
}
	...
	T: SendTransactionTypes<LocalCall>,
{
	...
	pub fn submit_transaction(
		...
		cessp_io::offchain::submit_transaction(xt.encode())
	}
...
pub struct Signer<T: SigningTypes, C: AppCrypto<T::Public, T::Signature>, X = ForAny> {
	...
	_phantom: cessp_std::marker::PhantomData<(X, C)>,
}
...
mod tests {
	...
	use codec::Decode;
	use cessp_core::offchain::{testing, TransactionPoolExt};
	use cessp_runtime::testing::{TestSignature, TestXt, UintAuthorityId};
	...
	fn should_send_unsigned_with_signed_payload_with_all_accounts() {
		...
		let mut t = cessp_io::TestExternalities::default();
