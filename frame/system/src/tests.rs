...
fn set_code_checks_works() {
	...
	impl cessp_core::traits::ReadRuntimeVersion for ReadRuntimeVersion {
	...
	for (spec_name, spec_version, impl_version, expected) in test_data.into_iter() {
		...
		ext.register_extension(cessp_core::traits::ReadRuntimeVersionExt::new(read_runtime_version));
		ext.execute_with(|| {
...
fn set_code_with_real_wasm_blob() {
	...
	let mut ext = new_test_ext();
	ext.register_extension(cessp_core::traits::ReadRuntimeVersionExt::new(executor));
	ext.execute_with(|| {
		System::set_block_number(1);
		System::set_code(
			RawOrigin::Root.into(),
