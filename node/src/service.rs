...
> {
	...
	let (block_import, rrsc_link) = cessc_consensus_rrsc::block_import(
		cessc_consensus_rrsc::Config::get_or_compute(&*client)?,
	...
	let import_queue = cessc_consensus_rrsc::import_queue(
		...
		move |_, ()| async move {
			...
			let slot =
				cessp_consensus_rrsc::inherents::InherentDataProvider::from_timestamp_and_duration(
					*timestamp,
					slot_duration,
				);
...
pub fn new_full(
	...
	let (_block_import, _rrsc_link) = cessc_consensus_rrsc::block_import(
		cessc_consensus_rrsc::Config::get_or_compute(&*client)?,
		grandpa_block_import,
		client.clone(),
	)?;
...
pub fn new_light(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	...
	let (rrsc_block_import, rrsc_link) = cessc_consensus_rrsc::block_import(
		cessc_consensus_rrsc::Config::get_or_compute(&*client)?,
		grandpa_block_import,
		client.clone(),
	)?;
