...
impl<'a> cessp_std::fmt::Display for HexDisplay<'a> {
	fn fmt(&self, f: &mut cessp_std::fmt::Formatter) -> Result<(), cessp_std::fmt::Error> {
		if self.0.len() < 1027 {
			for byte in self.0 {
...
impl<'a> cessp_std::fmt::Debug for HexDisplay<'a> {
	fn fmt(&self, f: &mut cessp_std::fmt::Formatter) -> Result<(), cessp_std::fmt::Error> {
		for byte in self.0 {
			f.write_fmt(format_args!("{:02x}", byte))?;
		}
...
impl AsBytesRef for cessp_std::vec::Vec<u8> {
	fn as_bytes_ref(&self) -> &[u8] {
		&self
	}
}
