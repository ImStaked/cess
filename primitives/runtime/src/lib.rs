...
#[doc(hidden)]
pub use cessp_application_crypto as app_crypto;

#[cfg(feature = "std")]
pub use cessp_core::storage::{Storage, StorageChild};
...
use cessp_std::{convert::TryFrom, prelude::*};
...
pub use cessp_application_crypto::{BoundToRuntimeAppPublic, RuntimeAppPublic};
...
pub use cessp_core::RuntimeDebug;

/// Re-export big_uint stuff.
pub use cessp_arithmetic::biguint;
/// Re-export 128 bit helpers.
pub use cessp_arithmetic::helpers_128bit;
/// Re-export top-level arithmetic stuff.
pub use cessp_arithmetic::{
...
impl IntoIterator for Justifications {
	...
	type IntoIter = cessp_std::vec::IntoIter<Self::Item>;
...
pub trait BuildStorage {
	...
	fn build_storage(&self) -> Result<cessp_core::storage::Storage, String> {
		let mut storage = Default::default();
		self.assimilate_storage(&mut storage)?;
		Ok(storage)
	}
	/// Assimilate the storage for this module into pre-existing overlays.
	fn assimilate_storage(&self, storage: &mut cessp_core::storage::Storage) -> Result<(), String>;
}
...
pub trait BuildModuleGenesisStorage<T, I>: Sized {
	...
	fn build_module_genesis_storage(
		&self,
		storage: &mut cessp_core::storage::Storage,
	) -> Result<(), String>;
}

#[cfg(feature = "std")]
impl BuildStorage for cessp_core::storage::Storage {
	fn assimilate_storage(&self, storage: &mut cessp_core::storage::Storage) -> Result<(), String> {
...
impl BuildStorage for () {
	fn assimilate_storage(&self, _: &mut cessp_core::storage::Storage) -> Result<(), String> {
...
impl traits::IdentifyAccount for MultiSigner {
	...
	fn into_account(self) -> AccountId32 {
		match self {
			...
			Self::Ecdsa(who) => cessp_io::hashing::blake2_256(who.as_ref()).into(),
		}
	}
}
	...
	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &AccountId32) -> bool {
		match (self, signer) {
			...
			(Self::Ecdsa(ref sig), who) => {
				let m = cessp_io::hashing::blake2_256(msg.get());
				match cessp_io::crypto::secp256k1_ecdsa_recover_compressed(sig.as_ref(), &m) {
					Ok(pubkey) =>
						&cessp_io::hashing::blake2_256(pubkey.as_ref()) ==
...
pub type DispatchResult = cessp_std::result::Result<(), DispatchError>;
...
pub type DispatchResultWithInfo<T> = cessp_std::result::Result<T, DispatchErrorWithPostInfo<T>>;
...
/// ```rust
/// # fn main() {
/// cessp_runtime::assert_eq_error_rate!(10, 10, 0);
/// cessp_runtime::assert_eq_error_rate!(10, 11, 1);
/// cessp_runtime::assert_eq_error_rate!(12, 10, 2);
/// # }
/// ```
///
/// ```rust,should_panic
/// # fn main() {
/// cessp_runtime::assert_eq_error_rate!(12, 10, 1);
/// # }
/// ```
#[macro_export]
#[cfg(feature = "std")]
...
impl cessp_std::fmt::Debug for OpaqueExtrinsic {
	#[cfg(feature = "std")]
	fn fmt(&self, fmt: &mut cessp_std::fmt::Formatter) -> cessp_std::fmt::Result {
		write!(fmt, "{}", cessp_core::hexdisplay::HexDisplay::from(&self.0))
	}

	#[cfg(not(feature = "std"))]
	fn fmt(&self, _fmt: &mut cessp_std::fmt::Formatter) -> cessp_std::fmt::Result {
		Ok(())
	}
}
...
impl ::serde::Serialize for OpaqueExtrinsic {
	...
	where
		S: ::serde::Serializer,
	{
		codec::Encode::using_encoded(&self.0, |bytes| ::cessp_core::bytes::serialize(bytes, seq))
	}
}
...
impl<'a> ::serde::Deserialize<'a> for OpaqueExtrinsic {
	...
	{
		let r = ::cessp_core::bytes::deserialize(de)?;
		Decode::decode(&mut &r[..])
...
impl SignatureBatching {
	...
	pub fn start() -> Self {
		cessp_io::crypto::start_batch_verify();
	...
	pub fn verify(mut self) -> bool {
		self.0 = true;
		cessp_io::crypto::finish_batch_verify()
	}
}
...
impl Drop for SignatureBatching {
	fn drop(&mut self) {
		...
		if !self.0 && !cessp_std::thread::panicking() {
...
mod tests {
	...
	fn batching_still_finishes_when_not_called_directly() {
		...
		ext.register_extension(cessp_core::traits::TaskExecutorExt::new(
			cessp_core::testing::TaskExecutor::new(),
		));

		ext.execute_with(|| {
			let _batching = SignatureBatching::start();
			cessp_io::crypto::sr25519_verify(&Default::default(), &Vec::new(), &Default::default());
		});
	}
