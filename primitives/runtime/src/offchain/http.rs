...
//!
//! `cessp-io` crate exposes a low level methods to make and control HTTP requests
...
//! Example:
//! ```rust,no_run
//! use cessp_runtime::offchain::http::Request;
//!
...
impl<'a, I: AsRef<[u8]>, T: IntoIterator<Item = I>> Request<'a, T> {
	...
	pub fn send(self) -> Result<PendingRequest, HttpError> {
		...
		let id = cessp_io::offchain::http_request_start(self.method.as_ref(), self.url, meta)
			.map_err(|_| HttpError::IoError)?;

		// add custom headers
		for header in &self.headers {
			cessp_io::offchain::http_request_add_header(id, header.name(), header.value())
				.map_err(|_| HttpError::IoError)?
		}

		// write body
		for chunk in self.body {
			cessp_io::offchain::http_request_write_body(id, chunk.as_ref(), self.deadline)?;
		}

		// finalize the request
		cessp_io::offchain::http_request_write_body(id, &[], self.deadline)?;
...
impl PendingRequest {
	...
	pub fn try_wait_all(
		...
		let statuses = cessp_io::offchain::http_response_wait(&ids, deadline.into());
...
impl Response {
	...
	pub fn headers(&mut self) -> &Headers {
		if self.headers.is_none() {
			self.headers = Some(Headers { raw: cessp_io::offchain::http_response_headers(self.id) });
		}
...
impl Iterator for ResponseBody {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		if self.filled_up_to.is_none() {
			let result =
				cessp_io::offchain::http_response_read_body(self.id, &mut self.buffer, self.deadline);
			match result {
				Err(e) => {
					self.error = Some(e);
					return None
...
mod tests {
	use super::*;
	use cessp_core::offchain::{testing, OffchainWorkerExt};
