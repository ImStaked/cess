...
use cessp_core::offchain::{Duration, Timestamp};
use cessp_io::offchain;
use cessp_std::fmt;
...
impl Lockable for Time {
	...
	fn snooze(deadline: &Self::Deadline) {
		...
		cessp_io::offchain::sleep_until(now.add(snooze));
	}
}
...
mod tests {
	use super::*;
	use cessp_core::offchain::{testing, OffchainDbExt, OffchainWorkerExt};
