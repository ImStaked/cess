...
impl RuntimeLogger {
	...
	pub fn init() {
		...
		log::set_max_level(cessp_io::logging::max_level().into());
	}
}
...
impl log::Log for RuntimeLogger {
	...
	fn log(&self, record: &log::Record) {
		use cessp_std::fmt::Write;
		let mut w = cessp_std::Writer::default();
		let _ = ::core::write!(&mut w, "{}", record.args());

		cessp_io::logging::log(record.level().into(), record.target(), w.inner());
	}
