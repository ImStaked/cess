...
use cessp_application_crypto::AppKey;
pub use cessp_arithmetic::traits::{
...
use cessp_core::{self, Hasher, RuntimeDebug, TypeId};
use cessp_std::{
	self,
...
impl IdentifyAccount for cessp_core::ed25519::Public {
	type AccountId = Self;
...
impl IdentifyAccount for cessp_core::sr25519::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl IdentifyAccount for cessp_core::ecdsa::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl Verify for cessp_core::ed25519::Signature {
	type Signer = cessp_core::ed25519::Public;

	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &cessp_core::ed25519::Public) -> bool {
		cessp_io::crypto::ed25519_verify(self, msg.get(), signer)
	}
}

impl Verify for cessp_core::sr25519::Signature {
	type Signer = cessp_core::sr25519::Public;

	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &cessp_core::sr25519::Public) -> bool {
		cessp_io::crypto::sr25519_verify(self, msg.get(), signer)
	}
}

impl Verify for cessp_core::ecdsa::Signature {
	type Signer = cessp_core::ecdsa::Public;
	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &cessp_core::ecdsa::Public) -> bool {
		match cessp_io::crypto::secp256k1_ecdsa_recover_compressed(
			self.as_ref(),
			&cessp_io::hashing::blake2_256(msg.get()),
		) {
...
impl<
		S: Verify<Signer = <<T as AppKey>::Public as cessp_application_crypto::AppPublic>::Generic>
			+ From<T>,
		T: cessp_application_crypto::Wraps<Inner = S>
			+ cessp_application_crypto::AppKey
			+ cessp_application_crypto::AppSignature
			+ AsRef<S>
			+ AsMut<S>
			+ From<S>,
	> AppVerify for T
where
	...
	<<T as AppKey>::Public as cessp_application_crypto::AppPublic>::Generic: IdentifyAccount<
		AccountId = <<T as AppKey>::Public as cessp_application_crypto::AppPublic>::Generic,
	>,
{
	type AccountId = <T as AppKey>::Public;
	fn verify<L: Lazy<[u8]>>(&self, msg: L, signer: &<T as AppKey>::Public) -> bool {
		use cessp_application_crypto::IsWrappedBy;
		let inner: &S = self.as_ref();
		let inner_pubkey =
			<<T as AppKey>::Public as cessp_application_crypto::AppPublic>::Generic::from_ref(&signer);
...
pub trait SimpleBitOps:
	Sized
	+ Clear
	+ cessp_std::ops::BitOr<Self, Output = Self>
	+ cessp_std::ops::BitXor<Self, Output = Self>
	+ cessp_std::ops::BitAnd<Self, Output = Self>
{
}
impl<
		T: Sized
...
impl Hasher for BlakeTwo256 {
	type Out = cessp_core::H256;
	type StdHasher = hash256_std_hasher::Hash256StdHasher;
	const LENGTH: usize = 32;

	fn hash(s: &[u8]) -> Self::Out {
		cessp_io::hashing::blake2_256(s).into()
	}
}

impl Hash for BlakeTwo256 {
	type Output = cessp_core::H256;

	fn trie_root(input: Vec<(Vec<u8>, Vec<u8>)>) -> Self::Output {
		cessp_io::trie::blake2_256_root(input)
	}

	fn ordered_trie_root(input: Vec<Vec<u8>>) -> Self::Output {
		cessp_io::trie::blake2_256_ordered_root(input)
	}
}
...
impl Hasher for Keccak256 {
	...
	fn hash(s: &[u8]) -> Self::Out {
		cessp_io::hashing::keccak_256(s).into()
	}
}
...
impl Hash for Keccak256 {
	...
	fn trie_root(input: Vec<(Vec<u8>, Vec<u8>)>) -> Self::Output {
		cessp_io::trie::keccak_256_root(input)
	}

	fn ordered_trie_root(input: Vec<Vec<u8>>) -> Self::Output {
		cessp_io::trie::keccak_256_ordered_root(input)
	}
}
...
impl CheckEqual for cessp_core::H256 {
	#[cfg(feature = "std")]
	fn check_equal(&self, other: &Self) {
		use cessp_core::hexdisplay::HexDisplay;
		if self != other {
			println!(
				"Hash: given={}, expected={}",
...
cessp_core::impl_maybe_marker!(
	...
	trait MaybeHash: cessp_std::hash::Hash;
	...
	Codec + Debug + Sync + Send + Clone + Eq + PartialEq + StaticTypeInfo
{
	...
	fn metadata() -> Vec<SignedExtensionMetadata> {
		cessp_std::vec![SignedExtensionMetadata {
...
impl SignedExtension for () {
	...
	fn additional_signed(&self) -> cessp_std::result::Result<(), TransactionValidityError> {
		Ok(())
	}
}
...
macro_rules! impl_opaque_keys_inner {
	(
		...
		impl $name {
			...
			pub fn generate(seed: Option<$crate::cessp_std::vec::Vec<u8>>) -> $crate::cessp_std::vec::Vec<u8> {
				let keys = Self{
					$(
						$field: <
							<
			...
			pub fn into_raw_public_keys(
				self,
			) -> $crate::cessp_std::vec::Vec<($crate::cessp_std::vec::Vec<u8>, $crate::KeyTypeId)> {
			...
			pub fn decode_into_raw_public_keys(
				encoded: &[u8],
			) -> Option<$crate::cessp_std::vec::Vec<($crate::cessp_std::vec::Vec<u8>, $crate::KeyTypeId)>> {
...
impl Printable for u64 {
	fn print(&self) {
		cessp_io::misc::print_num(*self);
	}
}

impl Printable for &[u8] {
	fn print(&self) {
		cessp_io::misc::print_hex(self);
	}
}

impl Printable for &str {
	fn print(&self) {
		cessp_io::misc::print_utf8(self.as_bytes());
	}
}
...
pub trait BlockNumberProvider {
	...
	///
	/// In case of using crate `cessp_runtime` with the crate `frame-system`,
...
mod tests {
	...
	use cessp_core::{crypto::Pair, ecdsa};

	mod t {
		use cessp_application_crypto::{app_crypto, sr25519};
		use cessp_core::crypto::KeyTypeId;
