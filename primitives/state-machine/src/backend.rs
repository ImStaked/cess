...
#[cfg(feature = "std")]
impl<'a, B: Backend<H>, H: Hasher> cessp_core::traits::FetchRuntimeCode
	for BackendRuntimeCode<'a, B, H>
{
	fn fetch_runtime_code<'b>(&'b self) -> Option<std::borrow::Cow<'b, [u8]>> {
		self.backend
			.storage(cessp_core::storage::well_known_keys::CODE)
			.ok()
			.flatten()
			.map(Into::into)
	}
	...
	H::Out: Encode,
{
	...
	pub fn runtime_code(&self) -> Result<RuntimeCode, &'static str> {
		let hash = self
			.backend
			.storage_hash(cessp_core::storage::well_known_keys::CODE)
			.ok()
			.flatten()
		...
		let heap_pages = self
			.backend
			.storage(cessp_core::storage::well_known_keys::HEAP_PAGES)
			.ok()
			.flatten()
