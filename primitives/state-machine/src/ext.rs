...
use cessp_core::storage::{well_known_keys::is_child_storage_key, ChildInfo, TrackedStorageKey};
use cessp_externalities::{Extension, ExtensionStore, Externalities};
	...
	N: crate::changes_trie::BlockNumber,
{
	...
	pub fn new(
		...
		backend: &'a B,
		changes_trie_state: Option<ChangesTrieState<'a, H, N>>,
		extensions: Option<&'a mut cessp_externalities::Extensions>,
	) -> Self {
		Self {
			overlay,
			backend,
	...
	N: crate::changes_trie::BlockNumber,
{
	...
	fn clear_prefix(&mut self, prefix: &[u8], limit: Option<u32>) -> (bool, u32) {
		...
		if cessp_core::storage::well_known_keys::starts_with_child_storage_key(prefix) {
			warn!(
				target: "trie",
	...
	N: crate::changes_trie::BlockNumber,
{
	...
	fn register_extension_with_type_id(
		...
		Err(cessp_externalities::Error::ExtensionsAreNotSupported)
	}
...
mod tests {
	...
	fn clear_prefix_cannot_delete_a_child_root() {
		...
		use cessp_core::storage::well_known_keys;
		let mut ext = ext;
