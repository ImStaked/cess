...
mod execution {
	...
	use cessp_externalities::Extensions;
		...
		N: crate::changes_trie::BlockNumber,
	{
		...
		pub fn new(
			...
			extensions.register(cessp_core::traits::TaskExecutorExt::new(spawn_handle));
...
mod tests {
	...
	impl CodeExecutor for DummyCodeExecutor {
		...
		>(
			...
			if self.change_changes_trie_config {
				ext.place_storage(
					cessp_core::storage::well_known_keys::CHANGES_TRIE_CONFIG.to_vec(),
			...
			match (using_native, self.native_succeeds, self.fallback_succeeds, native_call) {
				(true, true, _, Some(call)) => {
					let res = cessp_externalities::set_and_run_with_externalities(ext, || call());
	...
	impl cessp_core::traits::ReadRuntimeVersion for DummyCodeExecutor {
	...
	fn test_compact(remote_proof: StorageProof, remote_root: &cessp_core::H256) -> StorageProof {
	...
	fn runtime_registered_extensions_are_removed_after_execution() {
		use cessp_externalities::ExternalitiesExt;
