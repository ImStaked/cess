...
use cessp_externalities::{Extension, ExtensionStore, Extensions};
	...
	H::Out: Ord + 'static + codec::Codec,
{
	...
	pub fn execute_with<R>(&mut self, execute: impl FnOnce() -> R) -> R {
		...
		cessp_externalities::set_and_run_with_externalities(&mut ext, execute)
	}
	...
	pub fn execute_with_safe<R>(
		...
		std::panic::catch_unwind(move || {
			cessp_externalities::set_and_run_with_externalities(&mut *ext, f)
		})
...
impl<H, N> cessp_externalities::ExtensionStore for TestExternalities<H, N>
where
	H: Hasher,
	H::Out: Ord + codec::Codec,
	N: ChangesTrieBlockNumber,
{
	...
	) -> Result<(), cessp_externalities::Error> {
	...
	fn deregister_extension_by_type_id(
		...
		if self.extensions.deregister(type_id) {
			...
			Err(cessp_externalities::Error::ExtensionIsNotRegistered(type_id))
		}
	}
}

impl<H, N> cessp_externalities::ExternalitiesExt for TestExternalities<H, N>
where
	H: Hasher,
	H::Out: Ord + codec::Codec,
	N: ChangesTrieBlockNumber,
{
	...
	fn register_extension<T: Extension>(&mut self, ext: T) -> Result<(), cessp_externalities::Error> {
		self.register_extension_with_type_id(TypeId::of::<T>(), Box::new(ext))
	}

	fn deregister_extension<T: Extension>(&mut self) -> Result<(), cessp_externalities::Error> {
