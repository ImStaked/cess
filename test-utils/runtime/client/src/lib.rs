...
use cessp_runtime::traits::{Block as BlockT, Hash as HashT, HashFor, Header as HeaderT, NumberFor};
...
impl substrate_test_client::GenesisInit for GenesisParameters {
	fn genesis_storage(&self) -> Storage {
		...
		if let Some(ref code) = self.wasm_code {
			storage
				.top
				.insert(cessp_core::storage::well_known_keys::CODE.to_vec(), code.clone());
		}
...
pub fn new_light() -> (
	...
	let local_call_executor = client::LocalCallExecutor::new(
		backend.clone(),
		executor,
		Box::new(cessp_core::testing::TaskExecutor::new()),
		Default::default(),
	)
