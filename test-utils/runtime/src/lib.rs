...
use cessp_std::{marker::PhantomData, prelude::*};

use sp_application_crypto::{ecdsa, ed25519, sr25519, RuntimeAppPublic};
use cessp_core::{offchain::KeyTypeId, ChangesTrieConfiguration, OpaqueMetadata, RuntimeDebug};
use sp_trie::{
	...
	fn check(self) -> Result<Self, TransactionValidityError> {
		match self {
			...
			Extrinsic::Transfer { transfer, signature, exhaust_resources_when_not_first } =>
				if cessp_runtime::verify_encoded_lazy(&signature, &transfer, &transfer.from) {
...
impl cessp_runtime::traits::Dispatchable for Extrinsic {
	type Origin = Origin;
	type Config = ();
	type Info = ();
	type PostInfo = ();
	fn dispatch(self, _origin: Self::Origin) -> cessp_runtime::DispatchResultWithInfo<Self::PostInfo> {
...
pub type DigestItem = cessp_runtime::generic::DigestItem<H256>;
/// The digest of a block.
pub type Digest = cessp_runtime::generic::Digest<H256>;
/// A test block.
pub type Block = cessp_runtime::generic::Block<Header, Extrinsic>;
/// A test block's header.
pub type Header = cessp_runtime::generic::Header<BlockNumber, Hashing>;

/// Run whatever tests we have.
pub fn run_tests(mut input: &[u8]) -> Vec<u8> {
	use cessp_runtime::print;
...
impl frame_support::traits::PalletInfo for Runtime {
	fn index<P: 'static>() -> Option<usize> {
		let type_id = cessp_std::any::TypeId::of::<P>();
		if type_id == cessp_std::any::TypeId::of::<system::Pallet<Runtime>>() {
			return Some(0)
		}
		if type_id == cessp_std::any::TypeId::of::<pallet_timestamp::Pallet<Runtime>>() {
			return Some(1)
		}
		if type_id == cessp_std::any::TypeId::of::<pallet_babe::Pallet<Runtime>>() {
			return Some(2)
		}
...
fn code_using_trie() -> u64 {
	...
	let mut root = cessp_std::default::Default::default();
	let _ = {
		let v = &pairs;
		...
		impl_runtime_apis! {
			...
			impl sp_offchain::OffchainWorkerApi<Block> for Runtime {
				fn offchain_worker(header: &<Block as BlockT>::Header) {
					...
					cessp_io::offchain::submit_transaction(ex.encode()).unwrap();
				}
			}
			...
			impl sp_session::SessionKeys<Block> for Runtime {
				...
				fn decode_session_keys(
					encoded: Vec<u8>,
				) -> Option<Vec<(Vec<u8>, cessp_core::crypto::KeyTypeId)>> {
...
fn test_read_storage() {
	...
	cessp_io::storage::set(KEY, b"test");

	let mut v = [0u8; 4];
	let r = cessp_io::storage::read(KEY, &mut v, 0);
	assert_eq!(r, Some(4));
	assert_eq!(&v, b"test");

	let mut v = [0u8; 4];
	let r = cessp_io::storage::read(KEY, &mut v, 4);
	assert_eq!(r, Some(0));
	assert_eq!(&v, &[0, 0, 0, 0]);
}
...
fn test_read_child_storage() {
	...
	cessp_io::default_child_storage::set(STORAGE_KEY, KEY, b"test");

	let mut v = [0u8; 4];
	let r = cessp_io::default_child_storage::read(STORAGE_KEY, KEY, &mut v, 0);
	assert_eq!(r, Some(4));
	assert_eq!(&v, b"test");

	let mut v = [0u8; 4];
	let r = cessp_io::default_child_storage::read(STORAGE_KEY, KEY, &mut v, 8);
	assert_eq!(r, Some(0));
	assert_eq!(&v, &[0, 0, 0, 0]);
}
...
mod tests {
	...
	use cessp_core::storage::well_known_keys::HEAP_PAGES;
