...
fn check_signature(utx: &Extrinsic) -> Result<(), TransactionValidityError> {
	use cessp_runtime::traits::BlindCheckable;
...
fn execute_transaction_backend(utx: &Extrinsic, extrinsic_index: u32) -> ApplyExtrinsicResult {
	...
	match utx {
		...
		Extrinsic::OffchainIndexSet(key, value) => {
			cessp_io::offchain_index::set(&key, &value);
			Ok(Ok(()))
		},
		Extrinsic::OffchainIndexClear(key) => {
			cessp_io::offchain_index::clear(&key);
			Ok(Ok(()))
		},
...
fn execute_store(data: Vec<u8>) -> ApplyExtrinsicResult {
	let content_hash = cessp_io::hashing::blake2_256(&data);
	let extrinsic_index: u32 = storage::unhashed::get(well_known_keys::EXTRINSIC_INDEX).unwrap();
	cessp_io::transaction_index::index(extrinsic_index, data.len() as u32, content_hash);
	Ok(Ok(()))
}
...
fn info_expect_equal_hash(given: &Hash, expected: &Hash) {
	if given != expected {
		cessp_runtime::print("Hash not equal");
		cessp_runtime::print(given.as_bytes());
		cessp_runtime::print(expected.as_bytes());
	}
}
...
mod tests {
	...
	use cessp_io::{hashing::twox_128, TestExternalities};
	...
	fn block_import_works_wasm() {
		block_import_works(|b, ext| {
			...
			let runtime_code = RuntimeCode {
				code_fetcher: &cessp_core::traits::WrappedRuntimeCode(wasm_binary_unwrap().into()),
				hash: Vec::new(),
				heap_pages: None,
			};
